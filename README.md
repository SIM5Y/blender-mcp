...
The Hunyuan3D API URL is configured in the BlenderMCP sidebar panel.
...
- **Washed-out / greyed video after `render_sequence`**: fixed in v1.8.5 — the VSE encode now defaults to `view_transform="Standard"` so AgX/Filmic no longer tone-maps already-finished footage and text (it was greying white cards). The scene's real transform is restored after the encode; pass `view_transform="AgX"` only when grading raw 3D scene strips in the VSE.
- **`render_animation_preview` misses alpha fades / shows layers stacked**: the default fast preview uses the OpenGL/Workbench path, which is alpha-blind. For flat 2D or kinetic-typography clips with keyframed material Alpha/opacity, pass `engine="EEVEE"` (v1.8.5+) for an accurate per-frame render.
